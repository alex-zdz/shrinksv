sample_theta <- function(mu, phi, sigma, h0, h,  prior_spec ,expert){
  ...
  if(shrinksv == "volvol"){
  ar_prob <- ar_prob +
  (sigma^2 - sigma_prop^2) / (2 * 0.5 / prior_spec$sigma2_gamma_rate) # mistake in stochvol?!
  ...
  # Question: where does the 2 in 2 * 0.5 / prior_spec$sigma2_gamma_rate) come from?
  ...
  # const double ar_prob = 
  #  logacceptrateGamma(std::pow(sigma_prop, 2), std::pow(sigma, 2), 0.5 / prior_spec.sigma2.gamma.rate) +
  #  acceptance_rate_beta(mu, phi, sigma, gamma_prop, phi_prop, sigma_prop, h0, prior_spec, expert);

# that is because: prior_spec.sigma2.gamma.rate= 0.5 / priorsigma --> 0.5 / prior_spec.sigma2.gamma.rate = Bsigma
  ...
  if(log(runif(1))  < ar_prob){
    ...
    shrinksv <- prior_spec$shrinksv
    # calculate bT and BT:
    X <- matrix(c(rep(1,Ttot-1),c(h0,head(h,-1) )), Ttot-1 , 2)
    gamma_varinv <- 1/prior_spec$gamma_var
    phi_varinv <- 1/prior_spec$phi_var
    B0_inv <- diag(c(gamma_varinv,phi_varinv))
    BT_inv_11 <- Ttot-1 + B0_inv[1,1]
    ...
    quant_low <-  pnorm(-1, bT[2], chol22, lower.tail = TRUE ,log.p = FALSE)  # use chol11 = sqrt(BT[1,1]*sigma^2)
    quant_high <- pnorm(1, bT[2], chol22,  lower.tail = TRUE ,log.p = FALSE)
    phi_prop <- qnorm((quant_low + runif(1)*(quant_high-quant_low)), bT[2], chol22,lower.tail = TRUE ,log.p = FALSE)
    ...
    if(shrinksv == "volvol"){
      ar_prob <- ar_prob +
        (sigma^2 - sigma_prop^2) / (2 * 0.5 / prior_spec$sigma2_gamma_rate)
