sv_c_main <- function(y, draws, burnin , thinpara ,  prior_spec, startpara, startlatent
                      ){
    #set.seed(1)
    ...
    h0 <- startlatent$latent0 # contains h1 to hT, but not h0!
    h <- startlatent$latent
    ...
    latent_store <- matrix(NA , draws , Ttot-1)
    ...
    for( i in 1:niter){
      ...
      r <- sample_r(data , c(h0 , h ) , mix_prob , mix_mean , mix_var)
      ...
      parameter_draw <- sample_theta(mu, phi, sigma, h0, h, prior_spec , B0 )
